...
class Note(Base):
    ...
    id: UUID = sa.Column(SAUUID, primary_key=True, default=lambda: str(uuid4()))
    ...
    folder_id: UUID = sa.Column(SAUUID, sa.ForeignKey("folder.id"), nullable=True, index=True)
...
class NoteToNoteRelation(Base):
    ...
    parent_note_id: UUID = sa.Column(SAUUID, sa.ForeignKey("note.id"), nullable=False, index=True)
    parent_note = relationship(
        "Note", foreign_keys=[parent_note_id], back_populates="notes_relations"
    )

    child_note_id: UUID = sa.Column(SAUUID, sa.ForeignKey("note.id"), nullable=False, index=True)
...
class NoteTag(Base):
    ...
    note_id: UUID = sa.Column(SAUUID, sa.ForeignKey("note.id"), nullable=False, index=True)
